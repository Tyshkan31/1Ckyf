...
Процедура ПолучитьСообщенияТелеграм() Экспорт
	...
	Для каждого СообщениеОбъект ИЗ ПолученныеСообщения Цикл
		...
		ИначеЕсли Запись.ТекущееСостояние = СостояниеТекущиеОбращение Тогда
...
Процедура ДобавитьКомментарийКТекущемуОбращению(Комментарий, idЮзера)
	
	//Полуим последнее обращение пользователя
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
