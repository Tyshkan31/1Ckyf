...
Процедура ПолучитьСообщенияТелеграм() Экспорт
	...
	Для каждого СообщениеОбъект ИЗ ПолученныеСообщения Цикл
		Если СообщениеОбъект.message.Свойство("text") Тогда 	
			ТекстСообщения = СообщениеОбъект.message.text;
		ИначеЕсли СообщениеОбъект.message.Свойство("sticker") Тогда 
			ТекстСообщения = СообщениеОбъект.message.sticker.emoji;	
		Иначе
			Продолжить;
		КонецЕсли;
		...
		флЗакрыта = Ложь;
		Если НЕ НаборЗаписей.Количество() = 0 И НаборЗаписей[0].ПоследнееСообщение = ТекстСообщения Тогда
				Продолжить;		
		КонецЕсли;
		...
		ИначеЕсли ТекстСообщения = "/approve" Тогда //Нужно только установить пользователю состояние "Закрыто"
			...
			ИначеЕсли НаборЗаписей.Количество() = 0 
				ИЛИ НЕ НаборЗаписей[0].ТекущееСостояние = Перечисления.СостоянияПользователей.ОцениваетВыполнение Тогда
					ТекстСообщенияДляОтправки = "Невозможно выполнить действие.";
					ОтправитьСообщениеТелеграм(idЮзера, ТекстСообщенияДляОтправки);					
					Продолжить;
			КонецЕсли;
		...
		Запись.ПоследнееСообщение = ТекстСообщения;
		...
		ИначеЕсли флЗакрыта Тогда	                                     
			...
			|Пройдите пожалуйста опрос о качестве обслуживания. Ссылка на опрос:
			| https://app-959803902.1cmycloud.com/applications/serviceDesk-dev/survey?q=MDE5YTk3MzItYTI5ZC03ZTNhLTkzNTgtNzQ1MGY0NTJkMDU4";
...
Процедура ОтправитьСообщениеТелеграм(idПолучателя, ТекстСообщения, СостояниеПользователя = Неопределено, ПоследнееСообщение = Неопределено) Экспорт
	...
	Если Ответ.КодСостояния = 200 Тогда
		Если ЗначениеЗаполнено(СостояниеПользователя) Тогда
			...
			ПоследнееСообщение = НаборЗаписей[0].ПоследнееСообщение;
			НаборЗаписей.Очистить();
			Запись = НаборЗаписей.Добавить();
			Запись.idUser = idПолучателя;
			Запись.ТекущееСостояние = СостояниеПользователя;
			Запись.ПоследнееСообщение = ПоследнееСообщение;
			НаборЗаписей.Записать(Истина);
...
Процедура СоздатьНовуюЗаявку(ТекстСообщения, idЮзера) 
	...
	НовыйЭлемент.АвторОбращения = Формат(idЮзера, "ЧГ=0");
	...
	НовыйЭлемент.Записать();
	ТекстСообщения = "По вашему обращению была создана заявка!";
	ОтправитьСообщениеТелеграм(idЮзера, ТекстСообщения, СостояниеТекущиеОбращение); 
