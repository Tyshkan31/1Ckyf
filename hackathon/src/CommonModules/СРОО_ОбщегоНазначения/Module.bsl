...
Процедура ОтправитьОбращениеВЭлемент(СсылкаОбращения)
	...
	payload.Вставить("УуидЭлемента", "019abab8-5e9e-7ce2-9dcd-c92d4fe8e1e2");
	payload.Вставить("УуидПредприятия", Строка(СсылкаОбращения.УуидПредприятия));  
	...
	Если Токен <> Неопределено Тогда  
		...
		Соединение = Новый HTTPСоединение("app-959803902.1cmycloud.com",,,,,90,ЗащищенноеСоединение); // Или с параметром SSL, если требуется
