...
Процедура ОтправитьОбращениеВЭлемент(СсылкаОбращения) Экспорт
	...
	Если Токен <> Неопределено Тогда  
		...
		Заголовки.Вставить("Content-Type", НастройкиПодключения.Получить("ContentTypeОтправка"));
		Заголовки.Вставить("Authorization", СтрШаблон(НастройкиПодключения.Получить("АвторизацияОтправка"), Токен));
...
Функция ПолучитьТокен()
	...
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", НастройкиПодключения.Получить("ContentTypeТокен"));
	Заголовки.Вставить("Authorization", СтрШаблон(НастройкиПодключения.Получить("АвторизацияТокен"), Base64Auth));
