...
Функция СоздатьОбращениеВБазе(ТекстЗапроса, ОтветСервера, ЧатИД)
    ...
    Попытка
        ...
        НовоеОбращение.Источник = Перечисления.ИсточникиОбращений.Телеграм;
		...
		Если НовоеОбращение.Приоритет = Перечисления.ПриоритетыОбращения.Срочный Тогда
			
			МинимальныйТребуемыйБалл = 4;
			
		ИначеЕсли НовоеОбращение.Приоритет = Перечисления.ПриоритетыОбращения.Высокий Тогда
			
			МинимальныйТребуемыйБалл = 3;
			
		ИначеЕсли НовоеОбращение.Приоритет = Перечисления.ПриоритетыОбращения.Средний Тогда
		...
		Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыОбращения.ВРаботе);
		...
		Пока Выборка.Следующий() Цикл
			Если НЕ Выборка.СреднийБалл = NULL Тогда 
				Если Выборка.СреднийБалл > МинимальныйТребуемыйБалл Тогда
					...
					НовоеОбращение.Исполнитель = Выборка.Сотрудник;
